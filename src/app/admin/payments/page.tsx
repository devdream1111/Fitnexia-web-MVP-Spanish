...
export default function AdminPaymentsPage() {
  const { apiConnected } = useAdmin();
  const payments = apiConnected ? [] : MOCK_PAYMENTS;
  const totalCents = payments.reduce((sum, p) => sum + p.amount.amount, 0);
      ...
      {apiConnected ? (
        <div className="rounded-xl bg-[var(--fn-surface)] p-8 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--fn-primary-muted)]">
            <DollarSign size={28} className="text-[var(--fn-primary)]" />
          </div>
          <p className="text-lg font-semibold text-[var(--fn-text)]">Pagos procesados vía Mercado Pago</p>
          <p className="mt-2 text-sm text-[var(--fn-text-muted)]">
            El backend no expone un listado global de pagos para administradores. Los pagos de reservas y membresías se consultan en cada flujo correspondiente.
          </p>
        </div>
      ) : (
        <>
          <div className="rounded-xl bg-[var(--fn-surface)] p-6">
            <p className="text-sm text-[var(--fn-text-muted)]">Ingresos totales (vista demo)</p>
